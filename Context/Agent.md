...
- **AI**: Google Gemini (`GEMINI_API_KEY`) — NOT Anthropic
...
/backend
  ...
  src/
    ...
    aiFilter.js     # Gemini call: filter noise + classify + score
/frontend
  src/
    App.jsx
...
4. Send batch to Gemini with system prompt (see Context/AGENT_SPEC.md:94-106)
5. Filter `relevant:false`, sort by `importance` desc → `publishedAt` desc
6. Return array

## Fallback (no LLM key)
If `GEMINI_API_KEY` is empty/missing, skip step 4-5 and use keyword-overlap heuristic:
...
- `ANTHROPIC_API_KEY` from spec is **not used** — use `GEMINI_API_KEY` instead
